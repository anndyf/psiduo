...
import { autenticarAvaliacao } from "./actions"; // Importar a nova action
import LogoPsiDuo from "@/components/LogoPsiDuo";
import { ArrowRight, Lock, User } from "lucide-react";

export default function LoginAvaliacao() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await autenticarAvaliacao(cpf);
      
      if (res.success && res.id) {
        // Redireciona para a página específica do paciente com CPF na URL para autologin
        const cpfLimpo = cpf.replace(/\D/g, "");
        router.push(`/avaliacoes/${res.id}?cpf=${cpfLimpo}`);
      } else {
