...
export default function PainelPsicologo() {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        const [resPainel, resAv] = await Promise.all([
          buscarDadosPainel(),
          // buscarAvaliacoes precisa do psicologoId - vamos buscar depois
          Promise.resolve({ success: true, media: "0.0" })
        ]);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            ...(isDuoII ? [
              ...
              { 
                label: "Contatos", 
                desc: "Cliques no seu botão de WhatsApp.", 
                ...
                highlight: dados.cliquesWhatsapp,
            ...
            { label: "Suporte", desc: "Central de ajuda e assistência técnica.", icon: <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>, route: "/suporte", color: "border-purple-500" }
          ].map((item: any, idx) => (
            <div 
              key={idx}
